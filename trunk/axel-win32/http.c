...
#if WIN32
	if (INVALID_SOCKET == (conn->fd = tcp_connect(host, port, conn->local_if)))
#else
	if (-1 == (conn->fd = tcp_connect(host, port, conn->local_if)))
#endif
	{
		/* We'll put the message in conn->headers, not in request */
...
int http_exec(http_t *conn)
{
	...
	http_addheader(conn, "");
#if WIN32
	if (SOCKET_ERROR == send(conn->fd, conn->request, strlen(conn->request), 0)) 
	{
#ifdef DEBUG
